...
def show_doc():
    ...
    st.header('Global Temperature Climate Change Time Series Analysis and Forecasting Application : Documentation')
...
def show_forecast():
    ...
    lower_year = st.slider('Lower Year Range :',min_value=current_year,max_value=current_year + 20,value=current_year)
    upper_year = st.slider('Upper Year Range :',min_value=current_year,max_value=current_year + 20,value=current_year + 10)
    df = df.loc[str(lower_year):str(upper_year)]
    ...
    win = st.slider('Rolling Window [ Rolling Window of 1 is Equal to Real Data ] :',min_value=1,max_value=2*12,value=1)
    st.warning('Note : The below Trend Plot depend on the above rolling window. Set rolling window = 1, to get plot on Real Data as per above. Set rolling window > 1 [ 12 Recommended ] to view the trend.')

    
    str_show='##### Future Trend Plot on Average Temperature per Month with Moving Average of ' + str(win) + ' Months of Country : ' + country + ' and State : ' + state + ' from ' + str(lower_year) + ' to ' + str(upper_year)
    st.markdown(str_show,unsafe_allow_html=True)
    
    data = df['AverageTemperature'].rolling(window=win).mean()
    fig = px.line(data,title='Future Trend Plot', width=750, height=550)
    fig.update_layout(title={'font_size':25,'x':0.5})
    st.plotly_chart(fig)
...
''' 
    ...
    str_show='##### Future Seasonal Bar Plot on Average Temperature per Month of Country : ' + country + ' and State : ' + state + ' from ' + str(lower_year) + ' to ' + str(upper_year)
    st.markdown(str_show,unsafe_allow_html=True)
    

    temp = df['AverageTemperature'].groupby(df['AverageTemperature'].index.month).mean()
    temp.index = temp.index.map(month)
    fig = px.bar(temp,color=temp,title='Future Seasonal Bar Plot', width=750, height=550)
    fig.update_layout(title={'font_size':25,'x':0.5})
    st.plotly_chart(fig)
    ...
    str_show='##### Future Autocorrelation Plot on Average Temperature per Month of Country : ' + country + ' and State : ' + state + ' from ' + str(lower_year) + ' to ' + str(upper_year)
    st.markdown(str_show,unsafe_allow_html=True)
    
    acf_df = pd.DataFrame({'ACF':acf(df['AverageTemperature'],nlags=100,fft=False)})
    acf_df['Upper Level'] = 1.96 / 1.96 / (df.shape[0] ** 0.5 )
    acf_df['Lower Level'] = - 1.96 / 1.96 / (df.shape[0] ** 0.5 )
    fig = px.line(acf_df,title='Future Autocorrelation Plot')
    fig.update_layout(title={'font_size':25,'x':0.5})
    st.plotly_chart(fig)
